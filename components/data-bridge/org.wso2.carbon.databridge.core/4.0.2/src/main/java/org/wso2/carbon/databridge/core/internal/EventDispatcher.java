...
//        }
        ...
        synchronized (EventDispatcher.class) {
            ...
            if (log.isTraceEnabled()) {
                ...
                for (Map.Entry entry : streamTypeHolder.getDataTypeMap().entrySet()) {
                    logMsg += "StreamID=" + entry.getKey() + " :  ";
                    logMsg += "Meta= " + Arrays.deepToString(((AttributeType[][]) entry.getValue())[0]) + " :  ";
                    logMsg += "Correlation= " + Arrays.deepToString(((AttributeType[][]) entry.getValue())[1]) + " :  ";
                    logMsg += "Payload= " + Arrays.deepToString(((AttributeType[][]) entry.getValue())[2]) + "\n";
                }
                log.trace(logMsg);
            }
