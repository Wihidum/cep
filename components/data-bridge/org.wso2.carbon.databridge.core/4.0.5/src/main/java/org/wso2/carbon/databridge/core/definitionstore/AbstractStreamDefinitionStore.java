...
public abstract class AbstractStreamDefinitionStore implements StreamDefinitionStore {
    ...
    public boolean deleteStreamDefinition(Credentials credentials, String streamName, String streamVersion) {
        ...
        try {
            streamId = getStreamId(credentials, streamName,streamVersion);
        } catch (Exception e) {
            log.warn("Error when removing stream definition of " +  streamName+" "+streamVersion, e);
            return false;
        }
        if (streamId == null) {
            return false;
        ...
        removeStreamDefinition(credentials, streamId);
        return removeStreamId(credentials, constructNameVersionKey(streamName, streamVersion));
